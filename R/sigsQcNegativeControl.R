######################################################################################
#@description This function performs a negative control for the input signatures.
...
.sigQcNegativeControl <- function(genes, expressionMatrixList, outputDir, studyName, rePower=50, warningsFile, logFile){
  ...
  tryCatch({
    ...
    for(dataset.i in 1:datasets.num){
      ...
      utils::write.csv(x = neg.controls.summary,file = summary.filePath,row.names = TRUE, col.names = TRUE)
